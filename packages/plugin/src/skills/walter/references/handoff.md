...
### 2. Create or Update {{config_file}}

Single operation. If file exists, update it. If not, create it.

**Location:** Project root `{{config_file}}`

### 3. Keep It Minimal, Stay Flexible

{{config_file}} tells the next agent what to do NOW. Not project history. Not full documentation. Just:
...
{{config_file}} = current state. Context log = history. Most projects only need {{config_file}}.
...
- [ ] {{config_file}} exists at project root
